...
var indent = function(jbcleftright) {
	
	str = "";
	
	for(var i=0; i<jbcleftright; i++) {
			...
			$(data.list).each(
					function() {
						...
						str += "<li data-jbcidx='" + this.jbcidx 
					        + "' data-jbccontent='" + this.jbccontent
					        + "' data-jbcwriter='" + this.jbcwriter
					        + "' data-jbcupdown='" + this.jbcupdown
					        + "' data-jbcleftright='" + this.jbcleftright
					        + "' class='replyLi'>"
					        + indent(this.jbcleftright)
					        + "☞"+this.jbcidx + " : " + this.jbccontent
							+ " : " + this.jbcwriter
							+ " <button> 수정 </button> " 
							+ " <button> 댓글 </button></li>";
...
<script>	
	...
	$("#repliesDiv").on("click", ".replyLi button", function() {
		...
		var jbcupdown = reply.attr("data-jbcupdown");
		var jbcleftright = reply.attr("data-jbcleftright");
		...
		$("#jbcupdown").val(jbcupdown);
		$("#jbcleftright").val(jbcleftright);		
		$("#modDiv").show("slow");
	});
...
<div id='modDiv' style="display: none;">
	...
	<div>
		...
		<input type='hidden' id='jbcupdown'>
		<input type='hidden' id='jbcleftright'>		
	</div>
	<div>
...
<script>
	$("#comBtn").on("click",function(){
		...
		var jbccontent = $("#jbccontent").val();	
		var jbcwriter = $("#jbcwriter").val();	
		var jbcupdown = $("#jbcupdown").val();	
		var jbcleftright = $("#jbcleftright").val();	
		var jbcwritedate = "18/01/14";  //의미 없음. 콘트롤러에서 시스템 날자를 입력함
		var jbidx = ${jbv.jbidx};
		var jmidx = ${jbv.jmidx};
